...
Официант, принявший заказ онлайн, оставляет его у кухни. Если кухня свободна, она принимает заказ и
спустя 5–15 секунд возвращает уже готовое блюдо на выдачу. Время определяется случайным образом с помощью
функции std::rand() в указанных диапазонах.
Курьер приезжает каждые 30 секунд, забирает готовые блюда на выдаче и развозит их по заказчикам.
Программа завершается после 10 успешных доставок. На каждом из этапов, будь то заказ, или готовка, или
доставка заказа, выводятся соответствующие сообщения в консоль.*/
...
void waiter(void)
{
    ...
    while (true)
    {
        ...
        outputLock.lock();
        std::cout << "The order (" << order.id << ": " << order.dish << ") has been accepted and is awaiting cooking." << std::endl;
        outputLock.unlock();
    }
}
...
void chef(void)
{
    while (true)
    {
        ...
        outputLock.lock();
        std::cout << "The order (" << order.id << ": " << order.dish << ") has started to be cooking." << std::endl;
        outputLock.unlock();
        ...
        outputLock.lock();
        std::cout << "The order (" << order.id << ": " << order.dish << ") is ready and submitted for delivery." << std::endl;
        outputLock.unlock();
    }
}
...
void courier(void)
{
    while (true)
    {
        ...
        outputLock.lock();
        std::cout << "The order (" << order.id << ": " << order.dish << ") delivered." << std::endl;
        outputLock.unlock();
    }
}
